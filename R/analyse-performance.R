...
#' data(sensochoc) 
#' sensochoc %>%
#' specify(sensory_method = "QDA", panelist = panelist, product = product, session = session, pres_order = rank, attribute = cocoa_a:granular) %>% 
#' analyse_performance()
#' 
#' @export
analyse_performance <- function(.data) {
  ...
  tbl <- select(.data, panelist = meta_panelist,
