...
#'
#' @examples
#' generate_design(n_panelist = 10, product = letters[1:14], use_coding = FALSE)
...
generate_design <- function(n_panelist, product, use_coding = TRUE) {
  if (is.numeric(product) & length(product) == 1) {
    n_product <- product
    nms <- str_c("Product", seq_len(n_product))
  ...
  res <-
    ...
    set_colnames(str_c("Order", seq_len(n_product))) %>%
    mutate_all(~plyr::mapvalues(.x, seq_len(n_product), nms)) %>%
    mutate(Panelist = str_c("Panelist", seq_len(nrow(.)))) %>%
