fast_modified <- function(don, alpha = 0.05, sep.words = " ", word.min = 5, graph = TRUE, 
                   axes = c(1, 2), ncp = 5, B = 200, label.miss = NULL, ncp.boot = NULL, ...) 
{
  ...
  if (graph) {
    plot.MCA(res, choix = "ind", invisible = "ind", axes = axes, 
             new.plot = TRUE)
  }
  lev = rep(NA, J)
  for (i in 1:J) {
  ...
  if (graph) {
    res.boot <- boot(don, method = "sorting", ncp = ncp.boot, 
                     nbsim = B, level.conf = 1 - alpha, axes = axes)
  }
  ...
  if (graph) {
    dev.new()
    compt.words = as.table(restext$nb.words[(min(20, nrow(restext$nb.words))):1, 
                                            1])
    coordonnees = barplot(compt.words, las = 2, horiz = TRUE, 
                          names.arg = FALSE, main = "Occurrences of the most used words", 
                          sub = "Number of products characterized by the words")
    text(x = 1, y = coordonnees, labels = rownames(restext$nb.words)[(min(20, 
                                                                          nrow(restext$nb.words))):1], adj = 0)
  }
  ...
  res = list(eig = acm$eig, var = acm$var, ind = acm$ind, group = group_afm, 
             ...
             cramer = res2, textual = caract_prod, descriptor = data.frame(Occurrence = restext$nb.words[, 
                                                                                                         1], row.names = rownames(restext$nb.words)))
